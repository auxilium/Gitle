namespace Gitle.Model
{
  ...
  public class Issue : Touchable
  {
    ...
    {
      var result = Bookings.Where(x => x.IsActive && !x.Unbillable).Sum(x => x.Hours);
      return result;
    }
    ...
    {
      var result = BillableBookingHours() / TotalHours * 100;
      return result;
    }
    ...
    public virtual string ApplicationName(string project)
    {
      var application = Projects.Where(p => p.Name == project).Select(p => p.Application).ToString();
      return application;
    }
  }
}